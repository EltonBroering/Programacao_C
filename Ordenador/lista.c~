...
struct Nodo *atual, *ant, *prox, *antprox, *nodo1;
char *palavra;


	for(ant=NULL,atual=lista->primeiro;atual->proximo!=NULL;ant=atual,atual=atual->proximo)
	{
		for(antprox=atual,prox=atual->proximo;prox!=NULL;antprox=prox,prox=prox->proximo){
		if(strcasecmp(atual->palavra, prox->palavra) > 0){
			if(antprox!=NULL && ant!=NULL){
				antprox->proximo=atual;
				ant->proximo=prox;
      				nodo1 = prox->proximo;
